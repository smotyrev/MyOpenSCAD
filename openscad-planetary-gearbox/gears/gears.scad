    ...
    linear_extrude(height = width, twist = gamma){
        difference(){
            ...
            union(){
                tooth_width = (180*(1+clearance))/tooth_number+2*phi_r;
                circle(rf);                                         // Root Circle 
               // echo("Ring Gear Inner Diamater = ", rf);
                for (rot = [0:tau:360]){
                    rotate (rot) {                                  // Copy and Rotate "Number of Teeth"
                        polygon( concat(
                            [[0,0]],
...
//planetary_gear(modul=1, sun_teeth=9, planet_teeth=9, number_planets=3, width=5, rim_width=1, bore=5, pressure_angle=20, helix_angle=0, together_built=true, optimized=true);
