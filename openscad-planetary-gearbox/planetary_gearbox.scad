...
//---------------------------------------------------------------
show_carrier_top    =1; // display gearbox carrier top part with sun gear (0 - off, 1 - show as final, 2 - show with distancer)
...
show_motor_mount    =2; // display motor mounting base (0 - off, 1 - default, 2 - rs555)
...
////PLANET GEAR
PLANETS_N=3;
PLANET_GEAR_N=10;            // planet gear tooth count
PLANET_GEAR_H=4.7;          // planet gear height
PLANET_GEAR_ID=4.5;         // planet gear mountig hole inner diameter
...
//RING GEAR
RING_GEAR_N=30;             // ring gear tooth count
RING_GEAR_H=5;              // ring gear height/tickness
//GEARBOX
GEARBOX_SHAPE=2;            // 0-square shape, 1-circular, 2-octagonal
GEARBOX_H=9;                // gearbox height
GEARBOX_OD=47;              // gearbox outter diameter/width
GEAR_PREASURE_ANGLE=28;     // gear preasure angle
...
MOTOR_MOUNT_H=7;          // motor gear mount height 
MOTOR_MOUNT_OD=15;          // motor gear mount outter diameter
MOTOR_MOUNT_ID=5;         // motor gear mount inner diameter
...
MOUNTING_HOLE_P=24;         // position of the gearbox mounting holes
...
//DISTANCER
DISTANCER_H=3;              // distancer between individual geaboxes
//---------------------------------------------------------------
...
if(show_carrier_top)
{
    ...
    __distancer = show_carrier_top == 2 ? DISTANCER_H : 0;
    __gear_h_offset = show_carrier_top == 1 ? DISTANCER_H : 0;
    translate([0,0,t_offset])
    {

        //top gear
        color(color2)
        translate([0,0,GEARBOX_H+SUN_GEAR_DIST_H + __distancer])
        spur_gear (
            modul=GEAR_MODUL, 
            tooth_number=SUN_GEAR_N, 
            width=SUN_GEAR_H + __gear_h_offset, 
            bore=SUN_GEAR_ID, 
            pressure_angle=GEAR_PREASURE_ANGLE,   
            helix_angle=GEAR_HELIX_ANGLE, 
            optimized=true
        ...
        difference()
        {
            ...
            union()
            {
                ...
                translate([0,-0,h_top])
                cylinder(r=SUN_GEAR_DIST_D/2,h=SUN_GEAR_DIST_H+0.025 + __distancer);
...
} else if(show_motor_mount == 2) // RS555 motor
{
    m_offset = split ? 0 : 0;
    translate([0,0,m_offset])
    {
        difference()
        {
            union()
            {
                ...
                difference()
                {
                    ...
                    translate([0,0,MOTOR_BASE_H/1.5]) cylinder(r=ring_gear_id/2,h=MOTOR_BASE_H+0.2);
                    translate([0,0,-0.1]) cylinder(r=MOTOR_MOUNT_OD/2+0.5,h=MOTOR_BASE_H+0.2);
                    translate([0,12.5,-0.1]) cylinder(r=MOUNTING_HOLE_ID/2,h=MOTOR_BASE_H+0.2);
                    translate([0,-12.5,-0.1]) cylinder(r=MOUNTING_HOLE_ID/2,h=MOTOR_BASE_H+0.2);
                } 
            }
            rotate(a=(360/4/MOUNTING_HOLE_N),v=[0,0,1])
